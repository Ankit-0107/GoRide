  ...
  useEffect(() => {
    const fetchFollowers = async () => {
      try {
        ...
        if (res.data.success) setUsers(res.data.followers);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  return (
    <div className="bg-[#0e0e0e] text-white font-body min-h-screen">
      <header className="fixed top-0 w-full z-50 flex items-center justify-between px-6 h-16 backdrop-blur-xl bg-[#0e0e0e]/80 border-b border-white/5">
        ...
        <h1 className="font-bold text-sm tracking-wider uppercase">Followers</h1>
      ...
      <main className="max-w-xl mx-auto px-6 pt-20 space-y-2">
        {loading ? (
          ...
          <div className="text-center py-16 text-[#adaaaa] text-sm">No followers yet</div>
        ) : (
          users.map((u) => (
            <div
              key={u._id}
            ...
            >
              <div className="relative">
                <div className="w-11 h-11 rounded-full bg-gradient-to-br from-[#ff8f75] to-[#e6a7ff] flex items-center justify-center text-sm font-bold text-black shrink-0">
